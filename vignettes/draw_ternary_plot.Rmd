...
- Cartesian coordinates of the observations: The original Barycentric coordinates of the observations must be transformed into Cartesian coordinates using the Helmert matrix.
...
Please note that the order in which the labels are provided must match the order of the vertices in the ternary plot. The vertices are listed clockwise, from the right to the left, then ending at the top of the triangle. The best way is to get these labels from `ternable$alternatives` as `ternable` preserves the vertex orders.
